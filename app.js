...
async function checkAdminStatus() {
  try {
    ...
    if (isAdmin) {
      ...
      document
        .getElementById("copyContractAddress")
        .addEventListener("click", copyContractAddress);
    } else {
...
async function loadAdminRoyaltyInfo() {
  try {
    ...
    const percentage = ((Number(amount) * 100) / Number(samplePrice)).toFixed(
      2
    );
    ...
    if (receiver !== ethers.ZeroAddress && amount > 0n) {
      ...
      receiverElement.textContent = `${receiver.slice(0, 6)}...${receiver.slice(
        -4
      )}`;
      currentRoyaltyReceiver = receiver;
      currentRoyaltyPercentage = percentage;
      document.getElementById("copyRoyaltyAddress").style.display =
        "inline-block";
...
function setupAdminControls() {
  ...
  document
    .getElementById("setRoyaltyBtn")
    .addEventListener("click", setRoyalty);
  document.getElementById("withdrawBtn").addEventListener("click", withdraw);
  document
    .getElementById("copyRoyaltyAddress")
    .addEventListener("click", copyRoyaltyAddress);
  ...
  if (contract) {
    try {
      ...
      if (!contractURI) {
        ...
        // Decode base64 JSON
        const base64Data = contractURI.replace(
...
function copyRoyaltyAddress() {
  if (currentRoyaltyReceiver) {
    navigator.clipboard
      .writeText(currentRoyaltyReceiver)
      .then(() => {
...
async function searchNFT(tokenId = null) {
  ...
  try {
    ...
    nftDisplay.innerHTML =
      '<p class="error">Failed to load NFT information</p>';
  } finally {
    loadingIndicator.style.display = "none";
  }
}
...
function displayNFT(tokenId, isAvailable, owner, tokenURI, price) {
  ...
  const isOwner =
    userAddress && owner && owner.toLowerCase() === userAddress.toLowerCase();

  // OpenSea URL (Polygon network)
  const openSeaUrl =
    currentNetwork && currentNetwork.name === "Polygon"
        ...
        <div class="nft-info">
            ...
            <div class="nft-status">
                ...
                <span class="owner-info">
                    <span class="owner-address">${owner.slice(
                      0,
                      6
                    )}...${owner.slice(-4)}</span>
            ...
            ${
              isAvailable
                ? `<div class="nft-price">PRICE: ${ethers.formatEther(
                    price
                  )} ${nativeTokenSymbol}</div>`
                : ""
            }
            ...
            ${
              isOwner
                ? `<button class="buy-button send-button" onclick="openSendModal(${tokenId}, '${
                    metadata?.name || `Cat #${tokenId}`
                  }')">Send</button>`
                : isAvailable
        ...
        try {
          ...
          if (uri.startsWith("data:")) {
            try {
              ...
              console.error(
                `Failed to parse data URI for token ${tokenId}:`,
                e
              );
              continue;
            }
